  ...
  let rec aux (t : trm) : trm =
    match t.desc with
      | Trm_apps ({desc = Trm_prim (Prim_binop Binop_array_access);_} as p2, [{desc = Trm_var x;_}; index2], _) when (var_eq x array_var) ->
        ...
        let new_index = (trm_int (index_simpl mod new_size)) in
...
let extract_coeff (v: var) (t: trm): trm option =
  ...
  | Some (lhs, rhs) ->
    ...
    | Some (u) when (u.name = v.name) -> Some (trm_int 1)
    | _ -> None
    end
...
let loopize_decl ?(loop_ind_in = dummy_var) (loop_tg: target) (decl_tg: target): unit =
  ...
  Target.apply_at_target_paths (fun t ->
    let rec aux (t: trm): trm =
      ...
      | Some c -> coeff := c; if (loop_ind_in = dummy_var) then (trm_int 0) else (trm_get loop_ind_in)
...
let loopize_expr ?(loop_ind_in = dummy_var) (loop_tg: target) (new_acc_name: string) (expr_tg: target): unit =
    ...
    apply_at_target_paths_in_seq (fun i t ->
      begin match t.desc with
      | Trm_seq tl ->
        let new_decl = (trm_let (new_acc_var,typ_ptr (typ_int)) (trm_apps (trm_prim (Prim_ref typ_int)) [trm_int 0])) in
...
let _ = Run.script_cpp (fun () ->
  ...
  (* Reorder so that k is on the outside *)
  ...
  (* remove independent loops for im2col section *)
  ...
  (* Unroll inner tile loop for 4x registers*)
  ...
  (* TODO: had to add this custom rule because bind_multi was not cleaning up the nobraces afterwards *)
